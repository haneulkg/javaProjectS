...
<div class="container">
  ...
  <pre>
		...
		- 사용되는 메소드 ?
			1) encode (java.lang.CharSequence) : 패스워드를 암호화해주는 메소드이다. 반환타입은 String이다.
				 encode()메소드는 SHA-1에 의한 8Byte(64Bit)로 결합된 해시키를 랜덤하게 생성된 솔트(salt)를 지원한다.
  ...
  <div>
  	...
  	<input type="button" value="BCryptPassword암호화" onclick="bCryptPassword()" class="btn btn-outline-secondary" />
  	<input type="button" value="다시입력" onclick="location.reload()" class="btn btn-outline-secondary" />
  </div>
  <hr/>
  <div>
  	<div>출력결과 : </div>
