			...
			<tr>
				<td colspan="3" class="text-left">
					<c:if test="${flag != 'search'}">
						<input type="button" value="돌아가기" onclick="location.href='boardList?pag=${pag}&pageSize=${pageSize}';" class="btn btn-outline-success"/> 
					</c:if>
					<c:if test="${flag == 'search'}">
						<input type="button" value="돌아가기" onclick="location.href='boardSearch?pag=${pag}&pageSize=${pageSize}&search=${search}&searchString=${searchString}';" class="btn btn-outline-success"/> 
					</c:if>
					<c:if test="${sMid == vo.mid || sLevel == 0}">
						<input type="button" value="수정하기" onclick="location.href='boardUpdate?idx=${vo.idx}&pag=${pag}&pageSize=${pageSize}';" class="btn btn-outline-info"/>
						<input type="button" value="삭제하기" onclick="boardDelete()" class="btn btn-outline-warning"/>
					</c:if>
				</td>
				<c:if test="${sMid != vo.mid}">
					<td class="text-right"><input type="button" value="신고하기" onclick="complaintInput()" class="btn btn-outline-danger"></td>
				</c:if>
			</tr>
		</table>
		<!-- 이전글|다음글 처리하기 -->
			...
			<tr>
				<td>
					...
					<c:if test="${!empty preVo.title}">
						<a href="boardContent?idx=${preVo.idx}&pag=${pag}&pageSize=${pageSize}">이전글 : ${preVo.title}</a>
					</c:if>
				</td>
			</tr>
		</table>
			...
			<table class="table table-center">
				<tr>
					...
					<td style="width:15%">
						...
						<p><input type="button" value="댓글달기" onclick="replyCheck()" class="btn btn-outline-info btn-sm"/></p>
					</td>
				</tr>
			</table>
		</form>
