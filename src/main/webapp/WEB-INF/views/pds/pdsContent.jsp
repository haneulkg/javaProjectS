...
<head>
	...
	<style>
		...
		#starForm label:hover {
			text-shadow: 0 0 0 rgba(250, 200, 0);
		}
...
<jsp:include page="/WEB-INF/views/include/slide2.jsp" />
	...
	<div class="container">
		...
		<table class="table table-bordered text-center">
			...
			<tr>
				<td colspan="4"><input type="button" value="돌아가기" onclick="location.href='pdsList.pds?pag=${pag}&pageSize=${pageSize}&part=${part}';" class="btn btn-secondary form-control" /></td>
			</tr>
		</table>
		<hr/>
		<div>
			...
			<fieldset style="border: 0px">
				...
				<div class="m-0 p-0">
					<textarea name="review" id="review" rows="3" class="form-control" placeholder="별점후기를 남겨주시면 100포인트를 지급합니다"></textarea>
				</div>
				<div>
					<input type="button" value="별점|리뷰등록" onclick="reviewCheck()" class="btn btn-info form-control mt-1"/>
				</div>
			</form>
		</div>
		<hr/>
			...
			<div id="reviewBox">
				<div class="row">
					...
					<div class="col text-right">
						...
						<c:forEach var="i" begin="1" end="${5 - vo.star}" varStatus="iSt"><font color="lightgray">★</font></c:forEach>
					</div>
				</div>
		...
		<div class="text-center">
			<c:forEach var="fSName" items="${fSNames}" varStatus="st">
				...
				<c:if test="${extLower == 'jpg' || extLower == 'gif' || extLower == 'png'}">
					<img src="${ctp}/images/pds/${fSName}" width="85%"/><br/>
				</c:if>			
			</c:forEach>
		</div>
		<hr/>
